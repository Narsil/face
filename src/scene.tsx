...
export class THREEScene extends Component{
		...
		componentDidMount(){
				this.scene = new Scene(this.mount!.clientWidth, this.mount!.clientHeight)
				this.mount!.appendChild(this.scene!.renderer.domElement)
				var that = this
				// this.scene!.load(glass.filename).then((obj) =>{
				// 		that.scene!.add(obj)
				// })
				this.scene!.load(process.env.PUBLIC_URL + '/3dmodels/face.fbx').then((obj) =>{
						console.log("face obj", obj);
						// DEBUG: Add control points.
						for (var i = 0; i < 3; i++){
								const geometry = new THREE.BoxGeometry(.05, .05, .05);
								const cmaterial = new THREE.MeshBasicMaterial( {color: colors[i % colors.length]} );
